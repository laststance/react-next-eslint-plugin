...
/** Reuses a graph only for the same compiler and source snapshot; the rule adapter calls this per linted file. @example getHookOwnership(program, typescript).hooks */
  ...
  function protectModuleLoad(node) {
    ...
    if (!module && specifier && ts.isStringLiteralLike(specifier)) {
      ...
      if (
        ...
        !Object.keys(options.paths || {}).some((pattern) =>
          specifier.text.startsWith(pattern.split('*')[0]),
        )
      )
        return
    }
