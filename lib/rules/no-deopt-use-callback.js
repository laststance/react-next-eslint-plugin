export default {
  ...
  create(context) {
    ...
    function reportPassToIntrinsic(attributeNode, name) {
      ...
      context.report({ node: attributeNode, messageId: 'passToIntrinsic', data: { name: nameSuffix } });
    }

    function reportCalledInsideInline(attributeNode, name) {
      const nameSuffix = name ? ` "${name}"` : '';
      context.report({ node: attributeNode, messageId: 'calledInsideInline', data: { name: nameSuffix } });
    }
    ...
    return {
      ...
      JSXOpeningElement(node) {
        ...
        for (const attr of node.attributes || []) {
          if (!attr || attr.type !== 'JSXAttribute' || !attr.value || attr.value.type !== 'JSXExpressionContainer') continue;
      ...
      CallExpression(node) {
        ...
        const isIntrinsic = (typeArg.type === 'Literal' && typeof typeArg.value === 'string') || (typeArg.type === 'TemplateLiteral' && typeArg.quasis.length === 1);
