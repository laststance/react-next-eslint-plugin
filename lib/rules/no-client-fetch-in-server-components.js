export default {
  ...
  create(context) {
    ...
    function isClientFetchCallee(callee) {
      ...
      if (callee.type === 'Identifier' && (callee.name === 'axios' || callee.name === '$fetch')) return true;
      if (callee.type === 'MemberExpression' && callee.object && callee.object.type === 'Identifier' && (callee.object.name === 'axios')) return true;
      return false;
    }
    ...
    return {
      Program(node) {
        ...
        for (const stmt of node.body.slice(0, 3)) {
          if (stmt && stmt.type === 'ExpressionStatement' && stmt.expression && stmt.expression.type === 'Literal' && stmt.expression.value === 'use client') {
