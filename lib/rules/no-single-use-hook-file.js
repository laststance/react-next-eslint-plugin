...
export default {
  ...
  create(context) {
    ...
    try {
      ...
      throw new Error(
        'no-single-use-hook-file requires the optional TypeScript 6.0.x peer. Install typescript to enable this rule.',
      )
    }
    if (!/^6\.0\./.test(typescript.version)) {
      throw new Error(
        'no-single-use-hook-file currently supports TypeScript 6.0.x; use a supported compiler version.',
      )
    }
