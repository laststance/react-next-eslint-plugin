export default {
  ...
  create(context) {
    ...
    function collectSetterFromDeclarator(node) {
      ...
      if (node.id && node.id.type === 'ArrayPattern' && node.init && node.init.type === 'CallExpression' && isUseStateCallee(node.init.callee)) {
      ...
      if (expr.type === 'ObjectExpression') {
        for (const prop of expr.properties) {
          if (prop.type === 'Property' && !prop.computed) {
            const v = prop.value;
            if (v && v.type === 'Identifier' && setterNames.has(v.name)) return true;
          }
        }
      }
      return false;
    ...
    return {
      ...
      JSXAttribute(node) {
        ...
        if (expr && (expr.type === 'ArrowFunctionExpression' || expr.type === 'FunctionExpression')) return;
        if (isPassingSetterExpression(expr)) {
          let name = null;
          if (expr.type === 'Identifier') name = expr.name;
          context.report({ node, messageId: 'noPropDrillSetter', data: { name } });
        }
      },
      ...
      CallExpression(node) {
        ...
        const isCreateElement =
          ...
          (callee.type === 'MemberExpression' && !callee.computed && callee.property && callee.property.type === 'Identifier' && callee.property.name === 'createElement');
          ...
          if (isPassingSetterExpression(propsArg)) {
            ...
            if (propsArg.type === 'ObjectExpression') {
              const prop = propsArg.properties.find(
                (p) => p.type === 'Property' && p.value && p.value.type === 'Identifier' && setterNames.has(p.value.name)
              );
              if (prop && prop.value && prop.value.type === 'Identifier') name = prop.value.name;
            }
            context.report({ node, messageId: 'noPropDrillSetter', data: { name } });
          }
        }
