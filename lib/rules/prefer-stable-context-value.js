export default {
  ...
  create(context) {
    function isProviderElement(nameNode) {
      ...
      return property && property.type === 'JSXIdentifier' && property.name === 'Provider';
    }
    ...
    return {
      JSXOpeningElement(node) {
        ...
        for (const attr of node.attributes || []) {
          ...
          if (!attr.name || attr.name.type !== 'JSXIdentifier' || attr.name.name !== 'value') continue;
          if (!attr.value || attr.value.type !== 'JSXExpressionContainer') continue;
          const expr = attr.value.expression;
          const kind = literalKind(expr);
          if (kind) {
            context.report({ node: attr, messageId: 'preferStable', data: { kind } });
          }
        }
      },
    };
