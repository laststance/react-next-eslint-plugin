/**
 * @fileoverview Require useMemo for object/array props passed to memoized components.
 */
...
export default {
  meta: {
    ...
    docs: {
      description:
        'Ensure objects or arrays passed to memoized components are wrapped with useMemo to keep their identity stable.',
    ...
    messages: {
      inlineValue:
        'Memoized component "{{componentName}}" receives a new object/array for prop "{{propName}}". Wrap it with useMemo to keep the value stable.',
      unstableReference:
        'Value "{{valueName}}" passed to memoized component "{{componentName}}" via prop "{{propName}}" is not wrapped in useMemo. Wrap it with useMemo to prevent breaking memoization.',
    },
  },
    ...
    return {
      ...
      AssignmentExpression(node) {
        ...
        if (left.type === 'Identifier') {
          memoizedComponents.add(left.name)
        }
      },
      ...
      CallExpression(node) {
        ...
        const componentName =
          extractComponentNameFromCreateElementArg(componentArg)
